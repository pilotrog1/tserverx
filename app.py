...
def upload_catalog():
    ...
    try:
        ...
        if anuncio_data and isinstance(anuncio_data, dict):
            ...
            # Asegurar la existencia de campos clave y su tipo
            anuncio_data['active'] = bool(anuncio_data.get('active', False))
            anuncio_data['title'] = str(anuncio_data.get('title', ''))
            anuncio_data['description'] = str(anuncio_data.get('description', ''))
            anuncio_data['image'] = str(anuncio_data.get('image', ''))
...
# --- 6. Endpoint para Obtener el Anuncio (GET) (AJUSTADO) ---
@app.route('/advertisement', methods=['GET'])
def get_advertisement():
    
    # Formato por defecto completo
    default_ad_format = {"active": False, "title": "", "description": "", "image": ""}

    if AD_COLLECTION is None:
        # Devolver el formato por defecto si la DB no está lista
        return jsonify(default_ad_format), 200
    ...
    try:
        ...
        if ad_data:
            ...
            # Fusionar con el formato por defecto para asegurar que todos los campos existan
            # Esto es clave si un campo se añade después.
            response_data = {**default_ad_format, **ad_data}

            # Asegurar que 'active' sea booleano
            response_data['active'] = bool(response_data.get('active', False))

            return jsonify(response_data), 200
        else:
            # Si no hay documento, devuelve el objeto por defecto
            return jsonify(default_ad_format), 200
            
    except Exception as e:
        print(f"ERROR 500: Fallo al obtener el anuncio. Detalle: {e}")
        # En caso de error de DB, devolver el formato por defecto con un título de error
        error_response = default_ad_format.copy()
        error_response['title'] = "Error al cargar anuncio"
