...
from pymongo import MongoClient, UpdateOne, InsertOne, DeleteMany # ❗ Importaciones para bulk_write
...
# ----------------------------------------------------------------------
# 3. ENDPOINT: /catalog (CATÁLOGO - CORREGIDO)
# ----------------------------------------------------------------------
...
def get_catalog():
    ...
    try:
        ...
        for product in all_products:
            ...
            # ❗ CORRECCIÓN CRÍTICA DE MAPPING: Convertir campos de la DB a los esperados por Kivy.
            # Se asume: Kivy.rating = stars_count y Kivy.rating_count = favorites_count.
            
            stars_count = product.pop('stars_count', 0)
            favorites_count = product.pop('favorites_count', 0)

            # Asegurar que los datos sean numéricos (float para rating, int para count)
            try:
                product['rating'] = float(stars_count)
            except ValueError:
                product['rating'] = 0.0
            ...
            # Conversión de precio/oferta de vuelta a string (aunque Kivy puede manejar los números)
...
# ----------------------------------------------------------------------
# 5. ENDPOINT CRÍTICO: /upload (SUBIDA DE CATÁLOGO Y ANUNCIO - CORREGIDO)
# ----------------------------------------------------------------------
...
def upload_data():
    ...
    try:
        ...
        productos_a_eliminar = data.get('productos_a_eliminar', []) # ❗ IDs a eliminar
        ...
        # 2.1. BORRADO: Añadir operaciones de borrado para los IDs marcados en Kivy
        if productos_a_eliminar:
             valid_delete_ids = []
             for prod_id_str in productos_a_eliminar:
                  try:
                       # Solo borrar si es un ObjectId válido (es decir, un ID permanente)
        ...
        for prod in productos:
            ...
            # (Aunque Kivy ya los elimina, es buena práctica de servidor)
            ...
            if prod_id_str:
                try:
                    # Intentar convertir el ID de Kivy (P...) a ObjectId
                    mongo_id = ObjectId(prod_id_str)
                except Exception:
                    pass # No es un ObjectId válido, se tratará como nuevo.

            
            if mongo_id:
                # ❗ ACTUALIZACIÓN: Solo actualizar los campos editables usando $set.
                ...
                bulk_operations.append(
                    UpdateOne(
                        {'_id': mongo_id},
                        {'$set': update_doc},
                        upsert=False # Solo actualiza si existe
                    )
                )
            else:
                # ❗ INSERCIÓN DE NUEVO PRODUCTO
                # Inicializar las estadísticas para el nuevo producto
                prod['stars_count'] = 0
                prod['favorites_count'] = 0
                
                # El nuevo producto NO debe tener _id para que MongoDB lo genere
                prod.pop('_id', None) 
