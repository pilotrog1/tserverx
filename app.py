    ...
    try:
        ...
        if productos:
            ...
            for item in productos:
                if '_id' in item:
                     item.pop('_id') 
                # NOTA: No es necesario convertir price/offer aquí, se acepta el formato string/None de Kivy.
...
def get_catalog():
    ...
    try:
        ...
        for item in productos_list:
            # 1. Eliminar la clave interna de MongoDB (_id)
            if '_id' in item:
                item.pop('_id')
            ...
            if prod_id:
                # 2. CORRECCIÓN CLAVE: NO ELIMINAR 'id' de 'item'
                #    El cliente espera que 'item' (el valor) contenga su propia clave 'id'
