...
if MONGO_URI:
    try:
        # Configuración para manejar timeouts en la conexión
        ...
        # El comando ping es para verificar la conexión y autenticación.
...
def serve_image(filename):
    ...
    try:
        # send_from_directory maneja la ruta segura y el mimetype
...
# --- 4. Función de ayuda: Convertir valores a float si son válidos ---
def convert_to_float_or_empty(value):
    """Convierte un valor a float si es numérico, sino devuelve una cadena vacía (''). 
    Esto evita errores de tipo en MongoDB y mantiene los datos limpios."""
    ...
    return '' # Si no es un tipo esperado


# --- 5. Endpoint para Subir/Actualizar (Catálogo y Anuncio) (POST) ---
@app.route('/upload', methods=['POST'])
def upload_catalog():
    # Retorna 503 si la conexión a la DB falló al iniciar
    ...
    try:
        ...
        if not data or not isinstance(data, dict):
             # Respuesta 200 si no hay datos para procesar, no es un fallo del servidor
             return jsonify({"message": "Servidor activo. Operación de subida omitida (datos nulos o incorrectos)."}), 200
        ...
        # Borrar y reinsertar productos (actualización destructiva)
        CATALOG_COLLECTION.delete_many({})
        if productos:
            productos_a_insertar = []
            for item in productos:
                # Limpiar cualquier ID de MongoDB residual
                if '_id' in item:
                     item.pop('_id') 
                
                # Saneamiento de datos crucial para evitar errores en la DB
                item['price'] = convert_to_float_or_empty(item.get('price'))
                item['offer'] = convert_to_float_or_empty(item.get('offer'))
...
# --- 6. Endpoint para Obtener el Catálogo (GET) ---
...
def get_catalog():
    ...
    try:
        ...
        for item in productos_list:
            ...
            prod_id = item.get('id')
            if prod_id:
                # El ID se usa como clave en el diccionario para el cliente
                item.pop('id') 
...
# --- 7. Endpoint para Obtener el Anuncio (GET) ---
@app.route('/advertisement', methods=['GET'])
def get_advertisement():
    if AD_COLLECTION is None:
        # Devolver un objeto inactivo por defecto si la DB no está lista (503)
...
# --- 8. Punto de entrada para el servidor ---
if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    # Configuración de debug para desarrollo, debe ser False en producción
